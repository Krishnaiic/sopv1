...
import { unarchiveSop } from "@/services/documentApprovalService";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const result = await unarchiveSop(auth.actor, id, req);
  if (!result.ok) {
    const status =
  ...
  return NextResponse.json(ok({ unarchived: true }), { status: 200 });
}
