...
function buildImgHtml(params: { src: string; s3Key?: string; origin: "s3" | "url"; widthPct: number }) {
  ...
  const attrs: string[] = [
    ...
    `data-image-origin="${params.origin}"`,
  ];
  if (params.s3Key) {
    attrs.push(`data-s3-key="${params.s3Key.replace(/"/g, "&quot;")}"`);
  }
  return `<img ${attrs.join(" ")} />`;
}

export function SopImageModal({ open, onClose, onInsert }: Props) {
  ...
  const canInsert = useMemo(() => {
    ...
    if (tab === "url") return Boolean(imageUrl.trim());
    return Boolean(file);
  }, [busy, tab, imageUrl, file]);
  ...
  async function handleInsert() {
    ...
    if (tab === "url") {
      ...
      const html = buildImgHtml({ src, origin: "url", widthPct });
      onInsert({ html });
      onClose();
      return;
    }
    ...
    try {
      ...
      fd.set("file", file);
      const res = await fetch("/api/sop/images/upload", { method: "POST", body: fd });
      const data = (await res.json().catch(() => ({}))) as
        | { success: true; data: { key: string } }
        | { success: false; error: { message?: string } };
      if (!res.ok || (data as any).success === false) {
        setError((data as any).error?.message ?? "Failed to upload image.");
        setBusy(false);
        return;
      }
      const key = (data as any).data?.key as string;
      const src = `/api/sop/images/render?key=${encodeURIComponent(key)}`;
      const html = buildImgHtml({ src, s3Key: key, origin: "s3", widthPct });
      onInsert({ html });
      onClose();
    } finally {
      setBusy(false);
        ...
        <div className="border-b border-slate-200 bg-gradient-to-r from-sky-50 via-white to-teal-50 px-6 py-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-900">Add image</h2>
              <p className="mt-1 text-sm text-slate-600">
                Choose how you want to place an image into this section.
              </p>
            </div>
            <button
              type="button"
          ...
          {tab === "upload" ? (
            <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-sky-50/80 to-white p-5">
              <div className="mb-4 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    Pick an image file and insert it after upload.
                  </p>
                </div>
              </div>

              <label className="block rounded-2xl border border-dashed border-sky-200 bg-white px-4 py-4">
                <span className="text-xs font-semibold uppercase tracking-wider text-slate-500">Select image</span>
                <input
                  type="file"
                  accept="image/*"
            ...
            <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-teal-50/80 to-white p-5">
              <div className="mb-4 flex items-start gap-3">
                ...
                <div>
                  <h3 className="text-sm font-semibold text-slate-900">Use an image URL</h3>
                  <p className="mt-1 text-sm text-slate-600">
                    Paste a direct image link to place it into the section.
                  </p>
                </div>
              </div>
